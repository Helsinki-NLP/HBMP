...
class ModifiedHBMPEncoder(nn.Module):
    """
    Modified Hierarchical Bi-LSTM Max Pooling Encoder
    """
    def __init__(self, config):
        super(ModifiedHBMPEncoder, self).__init__()
        self.config = config
        self.max_pool = nn.AdaptiveMaxPool1d(1)
        self.rnn1 = nn.LSTM(input_size=config.embed_dim,
                            hidden_size=config.hidden_dim,
        ...
        self.rnn3 = nn.LSTM(input_size=config.embed_dim,
                            ...
                            dropout=config.dropout,
                            bidirectional=True)
        self.rnn4 = nn.LSTM(input_size=config.embed_dim,
                            hidden_size=config.hidden_dim,
                            num_layers=config.layers,
                            dropout=config.dropout,
                            bidirectional=True)
    ...
    def forward(self, inputs):
        ...
        out4, (ht4, ct4) = self.rnn4(inputs, (ht3, ct3))
        emb4 = self.max_pool(out4.permute(1,2,0)).permute(2,0,1)

        emb = torch.cat([emb2, emb3, emb4], 2)
